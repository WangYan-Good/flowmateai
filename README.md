...
`/api/messages` 必须能被 Azure 公网 HTTPS 访问。不要把 `/api/teams/message` 配成 Azure Messaging Endpoint，它不执行 Bot Framework 身份验证。

## 4. 使用 Tunnel 部署测试

本地服务默认监听 `0.0.0.0:8000`。Tunnel 只负责把公网 HTTPS 请求转发到本机，最终映射关系应为：
...
```bash
./start-linux.sh detached deepseek
curl http://localhost:8000/health
```

也可以用 `qwen` 替换 `deepseek`。Windows 使用 `start-windows.cmd` 启动。

### Cloudflare Quick Tunnel

安装了 `cloudflared` 的环境可直接执行：

```bash
cloudflared tunnel --url http://localhost:8000
```

命令会输出一个临时地址，例如 `https://random-name.trycloudflare.com`。另开终端验证：

```bash
curl https://random-name.trycloudflare.com/health
```

Quick Tunnel 每次重启后域名可能变化。域名变化时，需要同步更新 Azure Bot 的 Messaging endpoint，并用新域名重新打包 Teams 应用。

### ngrok

安装 ngrok、创建账号并配置 token 后执行：
...
从 ngrok 输出中取得 HTTPS 地址并验证：

```bash
curl https://YOUR_NAME.ngrok-free.app/health
```

需要频繁演示时，建议使用固定 ngrok 域名、Cloudflare Named Tunnel 或公司反向代理，避免反复更新 Bot 和 Teams 应用配置。
...
2. 绕过 Bot Framework 鉴权，验证模型和 Adaptive Card 生成：

   ```bash
   curl -X POST https://PUBLIC_HOST/api/teams/message \
     -H 'Content-Type: application/json' \
     -d '{"text":"summarize my emails"}'
   ```

3. 在 Azure Bot 的 Configuration 中设置真实回调入口：

   ```text
   https://PUBLIC_HOST/api/messages
   ```

4. 在 Teams 中向 Bot 发送 `summarize my emails`，完成端到端验证。

`/api/messages` 要求有效的 Bot Framework JWT。直接用普通 `curl` 请求该地址得到 `401` 并不代表 Tunnel 故障；应使用 `/health` 检查网络，用 `/api/teams/message` 检查业务链路。

不要把 API Key、Microsoft client secret 或 ngrok token 写入命令历史、Teams manifest 或提交到 Git。运行 Tunnel 的终端需要在测试期间保持开启。
...
Windows、Linux 和 macOS 均可使用项目自带脚本打包：
...
- 公网 `/health` 无法访问：确认本地 `/health` 正常、Tunnel 进程仍在运行，并检查 Tunnel 输出中的实际 HTTPS 域名。
